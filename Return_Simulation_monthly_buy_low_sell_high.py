...
def calculate_return_buy_low_sell_high_monthly_single_try_PER(sheet_tuple, PER_tuple, PER_threshold_low, PER_threshold_high, startdate, invest_years, monthly_income, inflation_rate=0):
...
def calculate_return_buy_low_sell_high_monthly_PER_iterate(ticker, PER_ticker, PER_threshold_low, PER_threshold_high, startdate: datetime, enddate: datetime, invest_years, monthly_income, inflation_rate=0, asset_log_dir =""):
    """
    ...
    while utils.canbe_simulated_yearly(sheet_tuple, current_date, invest_years) and (current_date + relativedelta(years=invest_years) < enddate):
        ...
        invested_amount, invested_asset, wallet, invested_months, sold_count, elapsed_months = \
            calculate_return_buy_low_sell_high_monthly_single_try_PER(
                sheet_tuple, PER_tuple, PER_threshold_low, PER_threshold_high, current_date, invest_years, monthly_income,
                                                                                                            inflation_rate)
...
def generate_dumpfiles_buy_low_sell_high_monthly_PER(
        ticker, PER_ticker, PER_threshold_low, PER_threshold_high, startdate: datetime, enddate, invest_years, monthly_income,
    ...
    asset_result_list, asset_list, invested_amount_list, invested_months_list, sold_count_list = \
        ...
        ticker, PER_ticker, PER_threshold_low, PER_threshold_high, startdate, enddate, invest_years, monthly_income, inflation_rate, asset_log_dir
        )
    ...
    for ticker in ["SPX"]:
        for PER_threshold_low in PER_threshold_low_list:
            for PER_threshold_high in PER_threshold_high_list:
                ...
                for invest_years in tqdm.tqdm([10, 20, 40, 50], desc=f"{ticker}, PER{PER_threshold_low} - {PER_threshold_high}"):
                    generate_dumpfiles_buy_low_sell_high_monthly_PER(
                        ticker, PER_ticker,PER_threshold_low, PER_threshold_high, startdate, enddate, invest_years, monthly_income, inflation_rate,
