...
out_dir = "graph_output/Return_Simulation/Return_Simulation_buy_after_falling/"
def plot_table_median(ticker, invest_years):

    change_rate_threshold_low_list = [i/100 for i in range(-10, 0, 1)]
    change_rate_threshold_high_list = [i/100 for i in range(1, 11, 1)] + [999]  # 99999999 means never sell
    table_data = []
    row = []
    for change_rate_threshold_low in change_rate_threshold_low_list[::-1]:
        row = []
        for change_rate_threshold_high in change_rate_threshold_high_list:

            dump_dir = f"Return_Simulation/Return_Simulation_Timing/buy_after_falling/dump/{ticker}_{invest_years}years_" \
                       f"changeRateLow_{change_rate_threshold_low}_" \
                       f"changeRateHigh_{change_rate_threshold_high}/"
    ...
    df = pandas.DataFrame(
        data=table_data,
        index = [f"{i:.0%}" for i in change_rate_threshold_low_list[::-1]],
        columns = [f"{i/100:.0%}" for i in range(1, 11, 1)] + ["Never sell"]
                     )
    utils.make_dir(out_dir + "table/")
    utils_plot.table_plot(df, f"{ticker} {invest_years}years (without dividends) buy after falling return\n"
...
if __name__ == '__main__':
    ...
    plot_table_summary("SPX", 30, [(-0.01, 0.01), (-0.05, 0.05), (-0.1, 0.1), (-0.1, 999)])
