...
asset_log_dir = "FIRE_Simulation\\asset_log\\"
...
def generate_simulation_datasets(sheet_tuple, startdate: datetime, enddate: datetime, withdraw_years, initial_asset,
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for i in tqdm.tqdm(range(iterate_start, 105, 5)):
...
def simulate_once_a_year_portfolio_single(sheet1, percentage1, sheet2, percentage2, startdate: datetime, withdraw_years,
                                          initial_assets, withdraw_rate, inflation_rate=0):
    # 条件：最初の一カ月経過時点で初めてwithdraw_rateで資産を取り出す(最初の取り出しは行わない）
    current_asset1 = initial_assets * percentage1
    current_asset2 = initial_assets * percentage2
    current_asset = current_asset1 + current_asset2

    current_date = startdate
    while get_index(sheet1, current_date) < 0:
        # current_dateがシート上にない場合(例：1926-01-03)は、次の日付があるまで繰り返す
        current_date += relativedelta(days=1)
    while get_index(sheet2, current_date) < 0:
        # current_dateがシート上にない場合(例：1926-01-03)は、次の日付があるまで繰り返す
        current_date += relativedelta(days=1)
    ...
    while current_date < startdate + relativedelta(years=withdraw_years):
        ...
        current_asset = int(
            (current_asset1 * (1 + growth_rate1)) + (current_asset2 * (1 + growth_rate2)) - withdraw_asset - (
                    current_asset * inflation_rate))
        # print("{}年後({})の資産額は、{}円。リターンは、{:.1%}".format(current_years, current_date.strftime("%Y/%m/%d"), cnum.jp(int(current_asset)), growth_rate), file=codecs.open(asset_logfile, "a", "utf-8"))
        datasets_years.append(current_years)
        datasets_assets.append(current_asset)
...
if __name__ == '__main__':
    ...
    for years in tqdm.tqdm(range(30, 70, 10)):
        generate_simulation_datasets(SPXL_tuple, startdate, enddate, years, 10000000, 0.02,
                                     "FIRE_Simulation\\SPXL100_{}years_inflation_2percent_dump\\".format(years),
                                     "FIRE_Simulation\\SPXL100_{}years_inflation_2percent_output\\".format(years),
    ...
    for years in tqdm.tqdm(range(30, 70, 10)):
        generate_simulation_datasets(SPX_tuple, startdate, enddate, years, 10000000, 0.02,
                                     "FIRE_Simulation\\SPX100_{}years_inflation_2percent_dump\\".format(years),
                                     "FIRE_Simulation\\SPX100_{}years_inflation_2percent_output\\".format(years),
