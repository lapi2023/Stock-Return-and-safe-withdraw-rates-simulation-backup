...
asset_log_dir = "FIRE_Simulation\\two_assets_no_rebalance_with_dividends\\asset_log\\"
...
def simulate_once_a_year_two_assets_single(sheet1, percentage1, sheet2, percentage2, startdate: datetime, withdraw_years,
                                           initial_asset, withdraw_rate, inflation_rate=0.02):
    # 条件：最初の1年経過時点で初めてwithdraw_rateで資産を取り出す(最初の取り出しは行わない）
    current_asset1 = int(initial_asset * percentage1)
    current_asset2 = int(initial_asset * percentage2)
    ...
    withdraw_asset = int(initial_asset * withdraw_rate)
...
def simulate_once_a_year_two_assets_multi(sheet1, percentage1, sheet2, percentage2, startdate: datetime,
                                          enddate: datetime, withdraw_years, initial_asset,
    ...
    while canbe_simulated(sheet1, current_date, withdraw_years) and (current_date < enddate):
        ...
        asset_final, final_asset1, final_asset2, simulated_years = simulate_once_a_year_two_assets_single(sheet1,
                                                                                                          percentage1,
                                                                                                          sheet2,
                                                                                                          percentage2,
                                                                                                          current_date,
        ...
        if asset_final <= 0:
            ...
            success_count += 1
            print(
                "Succeeded! assets: {}円 ({}年後の資産は{:.1f}倍になりました)".format(cnum.jp(int(asset_final)), withdraw_years,
                                                                              asset_final / initial_asset),
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for i in range(iterate_start, 625, 25):
            multi_results, simulated_count, success_count, asset_max_final, asset_best_startdate, asset_min_final, asset_worst_startdate = simulate_once_a_year_two_assets_multi(sheet1_tuple, percentage1, sheet2_tuple, percentage2, startdate,
                                                  enddate, withdraw_years, initial_asset, i / 10000, inflation_rate,
                                                  asset_log_dir
                                                  )
    ...
    for ticker1_ratio in tqdm.tqdm([0, .25, .5, .75, 1], desc=f"{ticker1} & {ticker2}, calculate different ratios: "):
        for years in range(30, 70, 10):
            generate_simulation_datasets_two_assets(tuple1, ticker1_ratio, tuple2, 1 - ticker1_ratio,
                                                    ...
                                                    f"FIRE_Simulation\\two_assets_no_rebalance_with_dividends\\{ticker1}{ticker1_ratio * 100}_{ticker2}{(1 - ticker1_ratio) * 100}_{years}years_inflation_{inflation_rate * 100}percent_dump\\",
                                                    f"FIRE_Simulation\\two_assets_no_rebalance_with_dividends\\{ticker1}{ticker1_ratio * 100}_{ticker2}{(1 - ticker1_ratio) * 100}_{years}years_inflation_{inflation_rate * 100}percent_output\\",
