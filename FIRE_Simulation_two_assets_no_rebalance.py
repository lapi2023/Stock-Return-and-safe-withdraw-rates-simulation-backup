...
SPXL_Sheets = r"Price Sheet\simulated_SPXL_1886-2023_monthly.csv"
SPX_Sheets = r"Price Sheet\simulated_SPX_funds_1886-2023_daily.csv"
SSO_Sheets = r"Price Sheet\simulated_SSO_1886-2023_monthly.csv"
BONDS_Sheets = r"Price Sheet\simulated_us_bonds_monthly.csv"
CASH_Sheets = r"Price Sheet\cash_monthly.csv"
asset_log_dir = "FIRE_Simulation\\two_assets_no_rebalance\\asset_log\\"
...
def simulate_once_a_year_two_assets_single(sheet1, percentage1, sheet2, percentage2, startdate: datetime,
    ...
    while current_date < startdate + relativedelta(years=withdraw_years) - relativedelta(months=1):
        ...
        if get_index(sheet1, current_date) < 0:
            print('Error128: date not found! {}'.format(current_date), file=sys.stderr)
    ...
    for ticker1_ratio in tqdm.tqdm([0, .25, .5, .75, 1], desc=f"{ticker1} & {ticker2}, calculate different ratios: "):
        for years in range(30, 70, 10):
            generate_simulation_datasets_two_assets(tuple1, ticker1_ratio, tuple2, 1 - ticker1_ratio,
                                                    ...
                                                    f"FIRE_Simulation\\two_assets_no_rebalance\\{ticker1}{ticker1_ratio * 100}_{ticker2}{(1 - ticker1_ratio) * 100}_{years}years_inflation_{inflation_rate * 100}percent_dump\\",
                                                    f"FIRE_Simulation\\two_assets_no_rebalance\\{ticker1}{ticker1_ratio * 100}_{ticker2}{(1 - ticker1_ratio) * 100}_{years}years_inflation_{inflation_rate * 100}percent_output\\",
