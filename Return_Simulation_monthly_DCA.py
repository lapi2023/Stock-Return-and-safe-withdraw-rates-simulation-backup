...
def calculate_return_dollar_cost_averaging_monthly_single_try(sheet_tuple, startdate, invest_years,
                                                              monthly_invest_amount, inflation_rate=0):
...
def calculate_return_dollar_cost_averaging_monthly_iterate(ticker, startdate: datetime, enddate: datetime, invest_years,
                                                           monthly_invest_amount, inflation_rate=0, asset_log_dir=""):
    """
    ...
    while utils.canbe_simulated_yearly(sheet_tuple, current_date, invest_years) and (
        ...
        invested_amount, asset, invested_months = calculate_return_dollar_cost_averaging_monthly_single_try(sheet_tuple,
                                                                                                            current_date,
                                                                                                            invest_years,
        ...
        print("{}円投資し、{}年後の資産は{}円({:.1f}倍)になりました".format(cnum.jp(int(invested_amount)), invest_years,
                                                                  cnum.jp(int(asset)), asset / invested_amount),
              file=codecs.open(asset_logfile, "a", "utf-8"))
...
if __name__ == '__main__':
    ...
    for ticker in ["SPX", "SPXL", "SSO"]:
        for invest_years in tqdm.tqdm([10, 20, 30, 40, 50], desc=f"{ticker}"):
            generate_dumpfiles_dollar_cost_averaging_monthly(ticker, startdate, enddate, invest_years,
                                                             monthly_invest_amount, inflation_rate,
