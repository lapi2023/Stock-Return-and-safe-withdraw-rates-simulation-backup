...
out_dir = "graph_output/Return_Simulation/buy_on_golden_cross/"
...
def plot_table_summary(ticker, invest_years, SMA_list):
    ...
    row_sold_count = []
    columns = []
    for SMA_short, SMA_long in SMA_list:
        dump_dir = f"Return_Simulation/Return_Simulation_Timing/buy_on_golden_cross/dump/{ticker}_{invest_years}years_" \
                   f"SMAshort_{SMA_short}_" \
                   f"SMAlong_{SMA_long}/"
        ...
        columns.append(f"SMA_short: {SMA_short}, SMA_long: {SMA_long}")
    ...
    utils_plot.table_plot(df, f"{ticker} {invest_years}years(without dividends) buy/sell on golden/dead crosses\n",
